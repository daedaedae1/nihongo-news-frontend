  ...
  return (
    <div className="container-xxl px-3 px-sm-4 px-md-5 my-4 my-md-5">
      <div className="card shadow-sm border-0 rounded-4">
        <div
          ref={containerRef}
          className="card-body p-3 p-md-4 p-lg-2"
          style={{ fontSize: "1.1rem", lineHeight: "1.75", position: "relative" }}
        >
          {/* 대제목 */}
          <h4 className="mb-1" style={{ letterSpacing: "-0.2px" }}>
            {titleTokens ? renderTokens(titleTokens) : news.title}
          </h4>
          {/* 대제목(한국어 번역) */}
          {krTitle && (
            <h4 className="mt-1" style={{ color: "#2a6", fontSize: "1.35rem" }}>
              {krTitle}
            </h4>
          )}
          ...
          <div className="text-center my-3">
            <img
              ...
              style={{ width: "30%", minWidth: 200, maxWidth: 360 }}
            />
          </div>
          ...
          {/* 요약(한국어 번역) */}
          <p className="fw-semibold mb-0" style={{ color: "#257", fontSize: "1.12rem" }}>
            {krDetail && krDetail.summary}
          </p>
          ...
          {/* 본문 */}
          {newsDetail.sections &&
            newsDetail.sections.map((section, idx) => (
              <div key={idx} className="mb-5">
                {/* 섹션 제목 */}
                <h5 style={{ color: "#367" }} className="mb-2">
                  {sectionTokens[idx]?.titleTokens?.length
                    ? sectionTokens[idx].titleTokens.map((t, i) => (
                        <span
                          key={i}
                ...
                {krDetail && krDetail.sections && krDetail.sections[idx] && (
                  <div className="mt-3">
                    <h5 className="mb-2" style={{ color: "#397", fontSize: "1.2rem" }}>
                      {krDetail.sections[idx].title}
                    </h5>
                    <pre
                      className="mb-0"
                      style={{ whiteSpace: "pre-wrap", color: "#257", fontSize: "1.1rem", lineHeight: 1.9 }}
                    >
                      {krDetail.sections[idx].body}
                    </pre>
                  </div>
          ...
          {popup.open && popup.token && (
            ...
            >
              ...
              <div className="d-flex justify-content-end gap-2 mt-2">
                <button className="btn btn-sm btn-outline-success" onClick={handleSaveClick}>
                  저장
                </button>
                <button className="btn btn-sm btn-outline-secondary" onClick={closePopup}>
                  닫기
                </button>
              </div>
            </div>
          ...
          {/* 팝업 바깥 클릭 닫기 */}
          {popup.open && <div onClick={closePopup} style={{ position: "fixed", inset: 0, zIndex: 5 }} />}
